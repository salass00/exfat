...
static int erase_object(struct exfat_dev* dev, const void* block,
		size_t block_size, fbx_off_t start, fbx_off_t size)
{
	const fbx_off_t block_count = DIV_ROUND_UP(size, block_size);
	fbx_off_t i;

	if (exfat_seek(dev, start, SEEK_SET) == (fbx_off_t) -1)
	{
...
static int create(struct exfat_dev* dev)
{
	...
	{
		position = ROUND_UP(position, (*pp)->get_alignment());
		if (exfat_seek(dev, position, SEEK_SET) == (fbx_off_t) -1)
		{
