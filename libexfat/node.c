 ...
 */
static fbx_off_t co2o(struct exfat* ef, cluster_t cluster, fbx_off_t offset)
{
...
static bool next_entry(struct exfat* ef, const struct exfat_node* parent,
		cluster_t* cluster, fbx_off_t* offset)
{
...
int exfat_flush_node(struct exfat* ef, struct exfat_node* node)
{
	cluster_t cluster;
	fbx_off_t offset;
	fbx_off_t meta1_offset, meta2_offset;
...
static bool erase_entry(struct exfat* ef, struct exfat_node* node)
{
	...
	fbx_off_t offset = node->entry_offset;
...
static int delete(struct exfat* ef, struct exfat_node* node)
{
	...
	fbx_off_t deleted_offset = node->entry_offset;
	int rc;
...
static int find_slot(struct exfat* ef, struct exfat_node* dir,
		cluster_t* cluster, fbx_off_t* offset, int subentries)
{
	struct iterator it;
	int rc;
	const struct exfat_entry* entry;
...
static int write_entry(struct exfat* ef, struct exfat_node* dir,
		const le16_t* name, cluster_t cluster, fbx_off_t offset, uint16_t attrib)
{
	struct exfat_node* node;
	struct exfat_entry_meta1 meta1;
	struct exfat_entry_meta2 meta2;
...
static int rename_entry(struct exfat* ef, struct exfat_node* dir,
		...
		fbx_off_t new_offset)
{
	struct exfat_entry_meta1 meta1;
	struct exfat_entry_meta2 meta2;
	cluster_t old_cluster = node->entry_cluster;
	fbx_off_t old_offset = node->entry_offset;
	const size_t name_length = utf16_length(name);
	const int name_entries = DIV_ROUND_UP(name_length, EXFAT_ENAME_MAX);
	int i;
...
static int find_label(struct exfat* ef, cluster_t* cluster, fbx_off_t* offset)
{
	struct iterator it;
	int rc;
