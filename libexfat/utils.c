...
{
	memset(stbuf, 0, sizeof(struct fbx_stat));
		...
		uint32_t free_clusters)
{
	...
	fbx_off_t total_space = le64_to_cpu(sb->sector_count) * SECTOR_SIZE(*sb);
	fbx_off_t avail_space = (fbx_off_t) free_clusters * CLUSTER_SIZE(*sb);
